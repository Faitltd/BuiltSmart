...
const { sendEstimateEmail, formatEstimateForEmail } = require('./emailService');
    ...
    if (completion.choices[0].message.function_call) {
      ...
      if (functionCall.name === 'updateConversationState') {
        ...
        if (args.detectedRooms && args.detectedRooms.length > 0) {
          ...
          args.detectedRooms.forEach((roomType) => {
        ...
        if (args.productSelections && args.productSelections.length > 0) {
          ...
          const products = args.productSelections.map((product) => ({
...
const getProductCategoriesForRoom = (roomType) => {
  switch (roomType.toLowerCase()) {
    case 'kitchen':
      return ['cabinets', 'countertops', 'appliances', 'flooring', 'lighting', 'fixtures'];
    case 'bathroom':
    case 'master bathroom':
      return ['cabinets', 'countertops', 'fixtures', 'tile', 'lighting'];
    case 'bedroom':
      return ['flooring', 'paint', 'lighting', 'doors'];
    case 'living room':
      return ['flooring', 'paint', 'lighting', 'windows'];
    case 'dining room':
      return ['flooring', 'paint', 'lighting'];
    case 'basement':
      return ['flooring', 'paint', 'lighting', 'doors', 'windows'];
    default:
